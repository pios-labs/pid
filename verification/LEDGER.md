...
| 19 | The dashboard facade serves real daemon state over HTTP (pure CLI consumer) | `examples/dashboard/server.mjs` (shells `pid … --json`) | `docs/dashboard-api.md` GET table | `/api/version`→`{api:1}`, `/api/services`→real list, `/api/budget/:n`→real `BudgetView` | `s8`: all three match the real daemon (paused `bud`, `tokensDay≥50`) | **verified** |
...
## CP6 — observability: the dashboard facade fed by a real run (`s8-dashboard.sh`)
