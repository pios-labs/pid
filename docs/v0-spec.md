...
| `pid_service_exit` | `signature` (`proc:spawn_error` = the command could not be spawned, `proc:exit_<code>` = exited non-zero), `code` (exit code or `null`), `signal` (terminating signal or `null`), `error?` (the spawn-error message) — written when a supervised process terminates **abnormally** (ADR 0012). A clean stop or signalled teardown writes no line (it was deliberate). The process emits no pi event in this case, so this synthetic line is the only chronicle record of a spawn failure or crash |
