...
3. **The rolling window is in-memory; the quarantine decision persists explicitly.** The not-yet-quarantined failure count lives in memory and resets if the daemon restarts; the terminal `quarantined` state is persisted to `state.json` as a sorted name-list, mirroring the existing `enabled[]` set. On boot the start-enabled routine skips-and-holds any quarantined service, structurally identical to how it already holds a budget-paused service. This durability is **not optional**: a quarantined service is still *enabled* (the user wants it running), so without persistence a daemon restart would silently restart the crash loop quarantine exists to stop.

   *(Mechanism note — corrected during the increment-B build: `state.json` does **not** serialize per-service `ServiceRecord.state`; it persists only name-lists, and runtime state is reconstructed on boot. The cost governor rebuilds `paused` by reading its budget file. The crash detector deliberately has **no** on-disk store, so there is nothing to reconstruct the quarantine *from* — which is exactly why the terminal bit must be persisted explicitly, as its own `quarantined[]` list.)*

   This is a **deliberate asymmetry** with the cost governor, which persists its numbers in full — and the asymmetry is principled: **money already spent is an unrecoverable fact** (forget it and a service blows its daily cap twice), whereas **a failure count is a self-regenerating signal** (if the service is still broken it re-trips the threshold in seconds). Different epistemics → different persistence: the governor reconstructs from a rich store; the detector persists one durable bit and lets the count regenerate.
